...
import {useAddMutation, useEditMutation, useDeleteMutation, useDeleteBulkMutation, useLazyParticularQuery, useLazyListQuery} from '../../redux/api/EventApi.js'

export default function AddEvent() {
    const [pageName] = useState([{
        title_1 : 'ADD EVENT',
        title_2 : 'DELETE SELECTED EVENT',
        title_3 : "EDIT EVENT",
        title_4 : "EVENT",
        title_5 : "ADD-MANAGE EVENT'S",
    }])
    ...
    const openEditModal = async (record_id) => {   
        ...
        getParticularRecord(record_id).then((response)=>{  
            setValue("event_title", response.data.data.event_title) 
            setValue("event_date_time", response.data.data.event_date_time)    
            setValue("event_description", response.data.data.event_description)    
                                                ...
                                                <div className="form theme-f">
                                                    <div className="row"> 
                                                        <div className="col-md-6 mb-3">
                                                            ...
                                                            <input className="form-control" type="text" placeholder="Event Title" {...register('event_title', { required: 'This field is required.' })}/>
                                                            {errors.event_title && <p className="text-danger error-custom-single">{errors.event_title.message}</p>}
                                                        </div> 
                                                        <div className="col-md-6 mb-3">
                                                            <label>Event Date</label>
                                                            <input className="form-control" type="date" placeholder="Event Date" {...register('event_date_time', { required: 'This field is required.' })}/>
                                                            {errors.event_date_time && <p className="text-danger error-custom-single">{errors.event_date_time.message}</p>}
                                                        </div> 
                                                        <div className="col-md-12 mb-3">
                                                            <label>Event Description</label>
                                                            <textarea className="form-control" type="text" placeholder="Event Description" {...register('event_description', { required: 'This field is required.' })}/>
                                                            {errors.event_description && <p className="text-danger error-custom-single">{errors.event_description.message}</p>}
                                                        </div> 
